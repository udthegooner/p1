...
//
// Author:  Deb Deppeler, Udhbhav Gupta
//
//////////////////////////// 80 columns wide //////////////////////////////////

/**
 * Test suite to run unit tests on Score and ScoreList classes
 *
 * <p>Bugs: None
 * 
 * @author Deb Deppeler, Udhbhav Gupta
...
public class Test_ScoreList {
	...
	/**
	 * Check that ScoreList is expanding correctly when full to add new Scores
	 */
	private static void test09_scoreList_expand(){
	    ScoreListADT list = new ScoreList();
	    Score s = new Score("Student", 5, 10);
	    try{
	        /* this will test expandArray if your initial array is 
	    	less than 100 or you can make your initial array less 
	    	than four and the original tests will work*/
	        for(int i = 0; i < 100; i++) {
	            list.add(s);
	        }
	        passed++;
	        ...
	        System.out.println("The ScoreList is not expanding at the"
	        		+ " right way.");
	        failed ++;
	    }
	    catch(Exception e) {
	...
	/**
	 * Check that multiple Scores are being correctly added and later
	 * removed from the ScoreList or not.
	 */
	...
	/**
	 * Check that a score added to a newly made ScoreList is being correctly
	 * removed and the list size after removing is 0.
	 */
	...
	/**
	 * Check that multiple Scores are being correctly added and later
	 * accessed from the ScoreList or not.
	 */
		...
		try {
			...
			for ( int i=0; i < scores.length ; i++ ) 
				if ( ! list.get(i).equals(scores[i])) 
					System.out.println("test06 get after adding scores "
							+ "to ScoreList failed");
			passed++;
		} catch (Exception e) {
		...
		try {
			for (int i=0; i < points.length; i++) {
				...
				if ( Math.abs(score.getMaxPossible() - possible[i]) > err ) 
					System.out.println("test03c ["+i+"] "+
					"score.getMaxPossible()=" +
